...
if (phoneInput) {
    phoneInput.addEventListener("input", () => {
        ...
        phoneInput.value = digits.length > 5 ? `+91 ${digits.slice(0, 5)} ${digits.slice(5)}` : digits;
    });
}
...
if (loginForm) {
    loginForm.addEventListener("submit", async (event) => {
        ...
        const phoneDigits = digitsOnly(phoneInput.value).replace(/^91(?=\d{10}$)/, "");
        ...
        if (!consent) {
            showLoginStatus("Please accept the development-prototype notice before continuing.");
            return;
        }
        ...
        formData.append("email_id", document.getElementById("email_id").value.trim());

        loginButton.disabled = true;
        loginButton.innerHTML = "Opening workspace <span>...</span>";
        ...
        try {
            ...
            loginButton.innerHTML = "Enter workspace <span>→</span>";
        }
    });
}
